...
package Interfaces.LPC43.SCU is
   ...
   type SFS_Register(Pin : Pin_Type := Normal_Drive) is record
      ...
      --  Select pin function.
      MODE     : MODE_ENUM := Interfaces.LPC43.SCU.Function_0_Default;

      --  Enable pull-down resistor at pad.
      EPD      : EPD_ENUM := Interfaces.LPC43.SCU.Disable_Pull_Down;

      --  Disable pull-up resistor at pad. By default, the pull-up resistor is
      --  enabled at reset.
      EPUN     : EPUN_ENUM := Interfaces.LPC43.SCU.Enable_Pull_Up;

      --  Input buffer enable. The input buffer is disabled by default
      --  at reset and must be enabled for receiving.
      EZI      : EZI_ENUM := Interfaces.LPC43.SCU.Disable_Input_Buffer;

      --  Input glitch filter. Disable the input glitch filter for
      --  clocking signals higher than 30 MHz.
      ZIF      : ZIF_ENUM := Interfaces.LPC43.SCU.Enable_Input_Glitch;

      --  Reserved
      RESERVED     : Interfaces.LPC43.UInt22 := 16#0#;

      case Pin is
         when Normal_Drive =>

            --  Reserved
            ND_EHS_RESERVED : Interfaces.LPC43.Bit := 16#0#;
            --  Reserved
            ND_EHD_RESERVED : Interfaces.LPC43.UInt2 := 16#0#;

         when High_Drive =>

            --  Select drive strength.
            EHD : EHD_ENUM := Interfaces.LPC43.SCU.Normal_Drive_4_Ma_D;
            --  Reserved
            HD_EHS_RESERVED : Interfaces.LPC43.Bit := 16#0#;

         when High_Speed =>

            --  Select Slew rate.
            EHS    : EHS_ENUM := Interfaces.LPC43.SCU.Slow_Low_Noise_With;
            --  Reserved
            HS_EHD_RESERVED : Interfaces.LPC43.UInt2 := 16#0#;
   ...
   type SFSUSB_Register is record
      ...
      USB_EPD    : SFSUSB_ENUM_2 :=
                    Interfaces.LPC43.SCU.Pull_Down_Disconnect;
      --  Reserved
      RESERVED   : SFSUSB_RESERVED_Field := 16#0#;
      --  Power mode.
      USB_EPWR   : SFSUSB_ENUM_3 :=
                    Interfaces.LPC43.SCU.Power_Saving_Mode_S;
      --  Enable the vbus_valid signal. This signal is monitored by the USB1
      ...
      USB_VBUS   : SFSUSB_ENUM_4 :=
                    Interfaces.LPC43.SCU.Vbus_Signal_Low_Or_I;
      --  Reserved
   ...
   type SFSI2C0_Register is record
      ...
      SCL_ZIF    : SFSI2C0_ENUM_3 :=
                    Interfaces.LPC43.SCU.Enable_Input_Filter;
